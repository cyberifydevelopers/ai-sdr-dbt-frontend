...
const API_PREFIX =
  (typeof import.meta !== "undefined" && import.meta.env?.VITE_API_PREFIX) ||
  (typeof process !== "undefined" && process.env?.NEXT_PUBLIC_API_PREFIX) ||
  "/api";

/** Frontend origin (used for OAuth return) */
const APP_ORIGIN = typeof window !== "undefined" ? window.location.origin : "";
/** Path in *this* SPA that the popup will land on (keep /integrations). */
const OAUTH_RETURN_PATH = "/integrations";

/** Only allow HubSpot everywhere (even if backend accidentally returns more). */
const ALLOWED_CRMS = ["hubspot"];
...
const fmtDT = (d) => {
  try {
    const dt = new Date(d);
    return isNaN(dt) ? "Unknown" : dt.toLocaleString();
  } catch {
    return "Unknown";
  }
};
...
/* ---------------------------------------------------------
   Buttons / Atoms (shared look & feel)
--------------------------------------------------------- */
function Button({
  children,
  className = "",
...
function RowSkeleton({ rows = 6, cols = 5 }) {
  return (
    <>
      {Array.from({ length: rows }).map((_, r) => (
        <tr key={r} className="animate-pulse">
...
function HubSpotMark({ size = 24 }) {
  return (
    <svg
      ...
      aria-label="HubSpot"
    >
      <circle cx="12" cy="12" r="9.5" stroke="#2563eb" strokeWidth="2" />
...
/* ---------------------------------------------------------
   Stat Card (safe sizes + truncation)
--------------------------------------------------------- */
function StatCard({ label, value, Icon, tone = "blue", valueClassName = "" }) {
  const ring = tone === "blue" ? "ring-blue-200/60" : "ring-cyan-200/60";
  const bg =
    tone === "blue" ? "from-blue-600 to-cyan-500" : "from-cyan-500 to-blue-600";

  return (
    <motion.div
      initial={{ y: 8, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      className="relative rounded-3xl border border-slate-200 bg-white p-4 sm:p-5 shadow-xl"
    >
      <div
        className="absolute -inset-0.5 rounded-3xl opacity-20 blur-2xl pointer-events-none"
        style={{ background: `linear-gradient(135deg, #06b6d466, #ffffff00)` }}
      />
      <div className="relative z-10 flex items-center gap-3 min-w-0">
        <div
          className={`flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${bg} text-white shadow-inner ring-1 ${ring} shrink-0`}
        >
          <Icon className="h-6 w-6 sm:h-7 sm:w-7" />
        </div>
        <div className="min-w-0">
          <div className="text-[10px] sm:text-xs uppercase tracking-wider text-slate-500">
            {label}
          </div>
          <div
            className={cx(
              "text-base sm:text-xl font-extrabold text-slate-900 truncate",
...
function Avatar({ name = "", size = 38 }) {
  ...
  return (
    <div
      className="overflow-hidden rounded-xl ring-1 ring-slate-200 bg-slate-100 grid place-content-center text-slate-700 font-bold shrink-0"
...
/* ---------------------------------------------------------
   Page
--------------------------------------------------------- */
export default function HubSpotIntegrationsPage() {
  const token = useRef(
  ...
  const hubspotConnected = accounts.some((a) => a.crm === "hubspot");
  const hubspotAccount = accounts.find((a) => a.crm === "hubspot");
  ...
  const fetchProviders = useCallback(async () => {
    try {
      ...
      setProviders(arr.filter((p) => ALLOWED_CRMS.includes(p?.name)));
    } catch (e) {
      console.error(e);
      setProviders([]);
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      const arr = Array.isArray(data) ? data : [];
      setAccounts(arr.filter((a) => a?.crm === "hubspot"));
    } catch (e) {
      console.error(e);
      setAccounts([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        await Promise.all([fetchProviders(), fetchAccounts()]);
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function startOAuth() {
    try {
      setBusy(true);
      const body = { redirect_to: `${APP_ORIGIN}${OAUTH_RETURN_PATH}` };
      const res = await authedFetch(EP.CONNECT("hubspot"), {
        method: "POST",
        body: JSON.stringify(body),
      });
      const json = await res.json();
      if (!res.ok || !json?.auth_url)
        throw new Error(json?.detail || "Failed to start OAuth");
      ...
      console.error(e);
      toast.error(e?.message || "OAuth failed to start");
      setBusy(false);
    }
  }
  ...
  async function saveToken() {
    if (!tokenFormValue.trim()) {
      toast.error("Token is required");
      return;
    }
    try {
      setSavingToken(true);
      const res = await authedFetch(EP.SET_TOKEN("hubspot"), {
        method: "POST",
        body: JSON.stringify({ access_token: tokenFormValue.trim() }),
      });
      const j = await res.json().catch(() => ({}));
  ...
  async function ensureFresh() {
    try {
      const res = await authedFetch(EP.ENSURE_FRESH("hubspot"), {
        method: "POST",
      });
      const j = await res.json().catch(() => ({}));
      if (!res.ok || j?.success === false)
  ...
  async function disconnect() {
    ...
    try {
      const res = await authedFetch(EP.DISCONNECT("hubspot"), {
        method: "DELETE",
      });
      const j = await res.json().catch(() => ({}));
      if (!res.ok || j?.success === false)
  ...
  async function fetchList(initial = false) {
    if (!hubspotConnected) {
      toast.info("Connect HubSpot first");
      return;
    }
    try {
      setLoadingItems(true);
      const base =
        mode === "contacts" ? EP.CONTACTS("hubspot") : EP.LEADS("hubspot");
      const params = new URLSearchParams({ limit: String(limit || 25) });
      let url = `${base}?${params.toString()}`;
      if (!initial && nextCursor) url += `&cursor=${encodeURIComponent(nextCursor)}`;
  ...
  const hubspotProvider = providers.find((p) => p.name === "hubspot");

  /* ---------------- UI ---------------- */
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden">
      {/* soft glows contained (no page overflow) */}
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute -top-28 -left-28 h-80 w-80 sm:h-96 sm:w-96 rounded-full bg-cyan-300/40 blur-3xl" />
        <div className="absolute -bottom-28 -right-28 h-80 w-80 sm:h-96 sm:w-96 rounded-full bg-blue-300/40 blur-3xl" />
      </div>

      <div className="mx-auto w-full max-w-screen-2xl px-3 sm:px-6 lg:px-10 py-5 sm:py-8 lg:py-10">
        {/* Header card */}
        <div className="rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 lg:p-7 shadow-xl relative overflow-hidden">
          <motion.div
            ...
            aria-hidden
          />
          <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between min-w-0">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
              <div className="grid h-10 w-10 sm:h-12 sm:w-12 place-content-center rounded-2xl bg-gradient-to-br from-white to-slate-50 ring-1 ring-slate-200 shadow-inner shrink-0">
                <HubSpotMark size={24} />
              </div>
              <div className="min-w-0">
                <h1 className="text-lg sm:text-2xl lg:text-3xl font-extrabold tracking-tight leading-tight truncate">
                ...
                <p className="text-[11px] sm:text-xs md:text-sm text-slate-600">
                  Connect HubSpot and browse Contacts & Leads directly in your app.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {!hubspotConnected ? (
                <>
                  ...
                  <Button onClick={() => setShowTokenModal(true)} size="sm">
                ...
                <>
                  <Button onClick={ensureFresh} size="sm">
          ...
          {/* Provider note */}
          {!hubspotConnected && (hubspotProvider?.type || "oauth") === "oauth" && (
            <div className="relative z-10 mt-3 flex items-start gap-2 text-[11px] sm:text-[12px] text-slate-500">
              <CircleAlert className="h-4 w-4 mt-0.5 text-slate-400 shrink-0" />
              <span className="min-w-0">
                Ensure backend env <code>ALLOWED_REDIRECT_ORIGINS</code> includes{" "}
                <code className="break-all">{APP_ORIGIN}</code>.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            label="Connection"
            value={hubspotConnected ? "Connected" : "Not connected"}
            Icon={hubspotConnected ? CheckCircle2 : XCircle}
            tone={hubspotConnected ? "blue" : "cyan"}
          />
          <StatCard
            label={mode === "contacts" ? "Contacts Loaded" : "Leads Loaded"}
            value={items.length}
            Icon={Users}
            tone="cyan"
          />
          <StatCard
            label="Next Cursor"
            value={nextCursor ? String(nextCursor).slice(0, 10) + "…" : "—"}
            Icon={Database}
            tone="blue"
          />
          <StatCard
            label="Account"
            value={hubspotAccount?.external_account_name || "—"}
            Icon={Shield}
            tone="cyan"
            valueClassName="truncate break-all max-w-[14ch] sm:max-w-[24ch] md:max-w-[28ch] xl:max-w-[32ch]"
          />
        </div>

        {/* Controls + Browser */}
        <div className="mt-5 rounded-3xl border border-slate-200 bg-white shadow-xl overflow-hidden">
          {/* Controls row */}
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between px-3 sm:px-5 py-3 border-b border-slate-200 bg-slate-50">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                disabled={!hubspotConnected || !nextCursor}
                size="sm"
              >
          ...
          {/* Desktop Table (≥xl) wrapped in overflow-x-auto to avoid page scroll */}
          {loadingItems ? (
            ...
            <div className="grid place-content-center py-14 px-4">
              <div className="grid h-16 w-16 place-content-center rounded-full bg-slate-100 mb-3">
            ...
            <>
                ...
                <div className="w-full overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filtered.map((it) => (
                        <tr
                          key={`${it.id}-${it.email || it.name || "row"}`}
              ...
              {/* Card Grid (<xl): 1 / 2 / 3 cols — no horizontal scroll */}
              <div className="xl:hidden p-3 sm:p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {filtered.map((it, idx) => (
                    <div
                      key={`${it.id}-${it.email || it.name || "card"}`}
                    ...
                    >
                      ...
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-0.5 flex items-center gap-1 text-[12px] sm:text-sm text-slate-600 min-w-0">
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-3 text-slate-700">
                            ...
                            <span className="inline-flex items-center gap-1 text-[11px] min-w-0">
                              <Building2 className="h-3.5 w-3.5 text-slate-500 shrink-0" />
          ...
          {/* Footer / paging + raw paging */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-3 sm:px-5 py-3 border-t border-slate-200 bg-slate-50">
            <div className="text-xs sm:text-sm text-slate-600 min-w-0">
              Next cursor:{" "}
              <b className="font-mono break-all">{String(nextCursor || "—")}</b>
            </div>
          ...
          {/* Raw paging: hidden on xs to prevent any x-scroll */}
          {Object.keys(rawPaging || {}).length > 0 && (
            <div className="hidden sm:block px-4 sm:px-6 py-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                ...
                <div className="max-w-full overflow-x-auto">
                  <pre className="text-[11px] text-slate-600 min-w-0">
                    {JSON.stringify(rawPaging, null, 2)}
                  </pre>
                </div>
              </div>
            </div>
      ...
      <AnimatePresence>
        {showTokenModal && (
          <Modal onClose={() => setShowTokenModal(false)} title="Set HubSpot PAT">
            <div className="space-y-3">
              <div className="text-xs text-slate-600">
                Paste your HubSpot Personal Access Token (PAT).
              </div>
              <input
                value={tokenFormValue}
                onChange={(e) => setTokenFormValue(e.target.value)}
              ...
              />
              <div className="rounded-xl border border-amber-200 bg-amber-50 text-amber-800 px-3 py-2 text-[12px] flex items-start gap-2">
                <CircleAlert className="h-4 w-4 mt-0.5" />
                Backend verifies via <code>/crm/token/hubspot</code>. Make sure CORS allows{" "}
                <code className="break-all">{APP_ORIGIN}</code>.
              </div>
            </div>
            <div className="mt-6 flex flex-wrap justify-end gap-2">
              <Button onClick={() => setShowTokenModal(false)} variant="subtle">
                Cancel
              </Button>
              <Button onClick={saveToken} variant="primary" disabled={savingToken}>
                {savingToken ? (
                  <>
...
function Th({ children }) {
  return (
    <th className="px-4 py-3 text-left text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-700">
      {children}
    </th>
  );
}
