...
/* ---------------- Typewriter Banner (same as user) ---------------- */
...
/* ---------------- Admin Sidebar (mirrors user sidebar) ---------------- */
export default function Sidebar() {
  ...
  // Responsive behavior (same thresholds)
  useEffect(() => {
  ...
  // Admin navigation items (admin paths kept)
  const menuItems = [
  ...
  const SidebarContent = () => (
    <>
      {/* Banner (same component/behavior) */}
      ...
      <nav className="flex-1 p-2 sm:p-4 space-y-1 sm:space-y-2 overflow-y-auto">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              {/* Neon rail when active (hidden in collapsed mode, like user) */}
  ...
  return (
    <>
      {/* Desktop/Tablet Sidebar (width transitions match user) */}
      {!isMobile && (
        <aside
      ...
      {/* Mobile Drawer (opened by header logo event) */}
      {isMobile && (
